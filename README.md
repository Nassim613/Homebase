...
**Finance**: add expense/income/transfer entries with category/store auto-fill and conditional fields (car, project, given-to, car-insurance split); categories & stores manager; day-grouped search; monthly net summary; offline-first with sync status.
...
**Weight**: You/Wife toggle, latest reading with change-since-last, full history.
...
**More/Settings**: paste your Google Sheet Web App URL here to turn sync on — **sync now covers Finance, Jazz, Weight, and Garage**, each writing to its own tab in your Sheet. Also includes a Cars & Projects manager (needed for Finance's conditional fields, and for Car Insurance's multi-car split) and a Garage Expense & Repair Types manager, plus quick links to the Finance and Jazz reports and a manual "Retry sync" button.
...
- Nicer entry-tap UI for Finance (currently uses `prompt()` for duplicate/edit/delete)
- Weight trend chart and time-range filters
